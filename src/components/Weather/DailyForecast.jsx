...
export default function DailyForecast({data}) {
    ...
    return (<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-4">
        {dayPeriods.map((day) => {
            const {
                number, name, // e.g., "Monday"
                temperature, temperatureUnit, windSpeed, windDirection, icon, // URL to icon image
                shortForecast,
            } = day;
            ...
            >
                ...
                {icon && (<img
                    src={icon}
                    alt={shortForecast}
                    className="w-16 h-16 sm:w-16 sm:h-16 mb-1"
                />)}
              ...
              {temperature}
                ...
                {windSpeed && (<span className="text-xs text-gray-500 mt-1">
